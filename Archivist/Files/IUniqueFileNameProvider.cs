...
{
    /// <summary>Возвращает уникальный путь для файла. Если файл существует, добавляется числовой суффикс.</summary>
    /// <param name="filePath">Путь к файлу.</param>
    /// <returns>Уникальный путь.</returns>
    string GetUnique(string filePath);
    /// <summary>Возвращает уникальный путь, исключая заданный набор путей.</summary>
    /// <param name="filePath">Путь к файлу.</param>
    /// <param name="excludedPaths">Набор исключаемых путей.</param>
    /// <returns>Уникальный путь.</returns>
    string GetUnique(string filePath, IReadOnlySet<string> excludedPaths);
    /// <summary>Находит все существующие файлы-дубликаты для указанного пути.</summary>
...
/// <summary>Реализация генерации имён с суффиксами <c>(1)</c>...<c>(100)</c>.</summary>
internal sealed class UniqueFileNameProvider : IUniqueFileNameProvider
{
    /// <summary>Абстракция файловой системы.</summary>
    private readonly IFileSystem _fileSystem;

    /// <summary>Инициализирует новый экземпляр <see cref="UniqueFileNameProvider"/>.</summary>
    ...
    public string GetUnique(string filePath, IReadOnlySet<string> excludedPaths)
    {
    ...
    /// <summary>Извлекает путь к каталогу из полного пути к файлу.</summary>
    /// <param name="filePath">Путь к файлу.</param>
    /// <returns>Путь к каталогу.</returns>
    private static string GetDirectory(string filePath) =>
        Path.GetDirectoryName(filePath)
        ?? throw new InvalidOperationException($"Не удалось определить каталог: {filePath}");
}